...
<div class="ratings">
    <div class="ratings-content">
      ...
      <div>
        <RatingsFilter :selectType="selectType" :onlyText="onlyText" />
      </div>

      <div class="rating-wrapper">
        <ul>
          <li class="rating-item" v-for="(rating, index) in filterRaings" :key="index">
            <div class="avatar">
...
export default {
  ...
  data () {
    return {
      onlyText: true, // 是否只有文本内容的评论
      selectType:2 // 查看的评论的类型
    }
  },
  async mounted(){
  ...
  computed:{
    ...
    filterRaings(){
      ...
      let filterRaings = []
      filterRaings = ratings.filter(rating => {
        const {rateType, text} = rating;
        // 默认显示推荐的评论 如果没有选中的话   并且默认会显示没有内容的评论 如果没有选择的话
